...
<body>
	 ...
	 <table>
	 	...
	 	<tbody>
		   ...
		   <c:when test="${empty requestScope.list}">
				<tr>
					<td colspan="7">현재 심사중인 청원이 없습니다.</td>
				</tr>
		   </c:when>
		   <c:otherwise>
				<c:forEach items="${requestScope.list}" var="list" varStatus="state">
					 ...
					 <td> 
					  ...
					  <td><a href="PetitionDetail.do?petition_no=${list.petition_no}&list=EvaluateList ">${list.title}</a></td>
					  <td>${list.writer}</td>
	...
	<div class="srcform">
		...
		<form action="EvaluateList.do" name="searchform" onsubmit="return searchClick()">  		<!-- 검색 -->
			<select name="src">
			...
			<c:if test="${startPage != 1}">
				<a href='EvaluateList.do?page=${startPage-1}&src=${src}&search=${search}'> 이전 </a>
			</c:if>
			...
			<c:forEach var="pageNum" begin="${startPage}" end="${endPage}">
				...
				<c:if test="${pageNum != spage}">
					<a href='EvaluateList.do?page=${pageNum}&src=${src}&search=${search}'> ${pageNum} </a>
				</c:if>
			</c:forEach>
			
			<c:if test="${endPage != maxPage }">
				<a href='EvaluateList.do?page=${endPage+1}&src=${src}&search=${search}'> 다음 </a>
			</c:if>
 																						   <!-- 검색결과 x -->
			<c:if test="${search != null}">
				<input type="button" value="되돌아가기" onclick="location='EvaluateList.do'">
			</c:if>
	 </div>
</body>
</html>